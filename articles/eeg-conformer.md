...
After spatial convolution batch normalization + ELU activation are applied. For the average pooling, the kernel size is $(1, 75)$ and the output dimensions is $k × 1 × T_out$, where T $T_out = floor((sp-24-75)/15) + 1$.
...
The attention module takes $X ∈ ℝ^(m×d)$ as input. $m$ is the number of tokens ($T_out$) and $d$ is the feature dimension per token (k = 40).
